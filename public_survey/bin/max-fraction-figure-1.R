...
# explore the relationships with max_fraction....
library(dplyr)
library(tidyr)
library(ggplot2)
library(glue)
...
from_same_species_and_cell_type <- sample_info$experiment[sample_info$organism==ORGANISM & sample_info$cell_type==CELL_TYPE & sample_info$is_single_cell]
from_same_species_and_cell_type <- metrics[metrics$library %in% from_same_species_and_cell_type,]
from_same_species_and_cell_type <- from_same_species_and_cell_type %>%
  dplyr::filter(metric %in% c('hqaa', 'max_fraction_reads_from_single_autosome')) %>%
  dplyr::mutate(value=as.numeric(value)) %>%
  tidyr::spread(key = metric, value = value)
lower_bound <- quantile(from_same_species_and_cell_type$max_fraction_reads_from_single_autosome, c(0.3))
upper_bound <- quantile(from_same_species_and_cell_type$max_fraction_reads_from_single_autosome, c(0.7))
MIN_NONOUTLIER_HQAA <- 20000
nonoutliers <- from_same_species_and_cell_type[from_same_species_and_cell_type$hqaa>=MIN_NONOUTLIER_HQAA & from_same_species_and_cell_type$max_fraction_reads_from_single_autosome<=upper_bound & from_same_species_and_cell_type$max_fraction_reads_from_single_autosome>=lower_bound,]
...
# circle the outlier...
# and mark out the non-outlier areas
p <- ggplot(metrics) +
  geom_point(aes(x = hqaa, y = max_fraction_reads_from_single_autosome, color=favored_chrom), alpha=0.3, stroke=0) + 
  geom_point(aes(x = hqaa, y = max_fraction_reads_from_single_autosome), shape=1, size=5, color='red', alpha = 1, data=metrics[metrics$library==OUTLIER,]) +
  geom_rect(aes(xmin=MIN_NONOUTLIER_HQAA, xmax=max(metrics$hqaa), ymin=lower_bound, ymax=upper_bound), alpha = 1, fill=NA, color='black', linetype='dashed') +
  scale_x_log10() +
  theme_bw() +
  xlab('Reads after filtering') +
  ylab('Max fraction reads from single autosome') +
  scale_color_viridis_d() +
  guides(color=guide_legend(title='Chrom.'))
pdf(glue('fig1-hqaa-vs-max-fraction-color-by-chrom.pdf'), height = 4, width = 4)
print(p)
dev.off()
