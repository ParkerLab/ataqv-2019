...
process ataqv_informative_names {
	...
	script:
	new_name = get_tn5(library) + ' Tn5, ' + get_replicate(library)	
...
process ataqv_metrics {
	...
	output:
        ...
        set file("fragment_length_distributions.txt"), file("tss_coverage.txt") into plot_fld_and_tss_enrichment_in
        file("metrics.txt") into plot_ataqv_metrics_in
        file("metrics.txt") into tn5_sensitive_peaks_metrics_in
...
process tn5_sensitivity_vs_tf_overlap_and_peak_size {
	...
	output:
	file('tn5_sensitivity_prob_by_chipseq_overlap_and_peak_signal.pdf')
	

	"""
	cat ${x.join(' ')} > master-peak-overlap.txt
	tn5-sensitivity-vs-tf-overlap-and-peak-size.R ${params.sample_info} $tn5_sensitivity master-peak-overlap.txt
	"""
