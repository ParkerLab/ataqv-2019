...
if (!is.null(opts$covariates_file)) {
	covariate_values <- read.table(opts$covariates_file, head = F, as.is = T, sep = '\t', col.names = c('file', 'metric', 'value')) %>%
	  dplyr::mutate(library=gsub("\\.json.*", '', basename(file))) %>%
	  dplyr::select(library, metric, value) %>% tidyr::spread(key = metric, value = value)
